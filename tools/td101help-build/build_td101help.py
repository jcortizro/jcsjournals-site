...
def build_css():
  ...
  * { box-sizing: border-box; }
  ...
  /* Carrd paints its site-level background texture on body::before, which no
     rule scoped to body's CHILDREN can reach. MEASURED live on the walkthrough site (circlehelp.carrd.co, then named testjc)
     2026-09-03: with every old element deleted and the computed background on
     html and body both reading clean cream, the page still rendered as grey
     noise, and the culprit was this pseudo-element. It costs no bandwidth, it
     just makes the page look broken. Killing it here rather than in the Carrd
     embed keeps the rule in the repo, so a fix is a push and never a re-paste. */
